...
def collect_and_transform(df, batchName):
	...
	smTble = df.sample(frac=plotFraction)
	...
	origVals = sns.boxplot(x=grouping_column, y='value', color="#CD7F32", data=df_melted, ax=ax1, showfliers = False)
	...
	#smTble = bcDf.groupby(grouping_column).apply(lambda x: x.sample(frac=plotFraction)) 
	smTble = df.sample(frac=plotFraction)
	df_batching = smTble.filter(regex=f"(Mean|Median|{grouping_column})",axis=1)
	df_melted = pd.melt(df_batching, id_vars=[grouping_column])
	fig, ax1 = plt.subplots(figsize=(20,8))
	origVals = sns.boxplot(x=grouping_column, y='value', color="#50C878", data=df_melted, ax=ax1, showfliers = False)
	plt.setp(ax1.get_xticklabels(), rotation=40, ha="right")
	...
	for i in range(0, len(colNames), 4):
		...
		# pdfOUT.savefig(fig)
	bcDf.to_csv("{}_boxcox_mod.tsv".format(batchName), sep="\t") 	
...
if __name__ == "__main__":
    ...
    myFileIdx = "${quant_table}".split("_")[0]
